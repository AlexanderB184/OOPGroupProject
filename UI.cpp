...
void UI::selectCharacter(Controller * Player) { 
    ...
    Moves1[2] = new Heal_Move("Meditate", 6, 30, 2);
    ...
    Moves2[2] = new Heal_Move("Eat", 10, 30, 2);
    possibleCharacters[1] = new Character("Jane",200,45,100,20,40,Moves2,3);
    Move **Moves3 = new Move *[3];
    Moves3[0] = new Attack_Move("Stab", 1000, 30, 80);
    Moves3[1] = new Heal_Move("t", 1000, 20, 0);
    Moves3[2] = new Heal_Move("T", 1, 100, 0);
    ...
    Moves4[2] = new Heal_Move("Eat", 10, 30, 2);
    ...
    while (!isValid) {
        ...
        for (int iChar = 0; iChar < nChar; iChar ++) {
            if (possibleCharacters[iChar][0].Name == selectedCharacter) {// || iChar + 1 == to_string(selectedCharacter)) {
              isValid = true;
